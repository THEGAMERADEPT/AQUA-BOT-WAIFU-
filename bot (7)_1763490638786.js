...
bot.onText(/\/upload/, async (msg) => {
    ...
    try {
        ...
        await pool.query('UPDATE waifus SET price = $1 WHERE price = 0 OR price IS NULL', [price]);
        ...
        if (uploadNotificationGroup) {
            try {
                await bot.sendPhoto(uploadNotificationGroup, fileId, {
                    caption: `⚠️ 𝗧𝗛𝗜𝗦 𝗪𝗔𝗜𝗙𝗨 𝗛𝗔𝗦 𝗕𝗘𝗘𝗡 𝗨𝗣𝗟𝗢𝗔𝗗𝗘𝗗, 𝗡𝗢 𝗢𝗡𝗘 𝗦𝗛𝗢𝗨𝗟𝗗 𝗨𝗣𝗟𝗢𝗔𝗗 𝗜𝗧 𝗡𝗢𝗪.`,
                    parse_mode: 'HTML'
                });
            } catch (e) {
        ...
        await sendPhotoReply(msg.chat.id, msg.message_id, fileId, 
            `✅ <b>Waifu Successfully Added to Collection!</b>\n\n𝗡𝗔𝗠𝗘: ${name}\n𝗔𝗡𝗜𝗠𝗘: ${anime}\n𝗥𝗔𝗥𝗜𝗧𝗬: ${RARITY_NAMES[rarity]}\n𝗜𝗗: ${waifuId}\n𝗣𝗥𝗜𝗖𝗘: ${price}💸`
        );
    } catch (error) {
...
bot.on('message', async (msg) => {
    ...
    if (tracker.rows.length === 0) {
        await pool.query('INSERT INTO spawn_tracker (group_id, message_count) VALUES ($1, 1)', [groupId]);
        return;
    }
    ...
    if (!activeSpawn && currentCount >= 100) {
        ...
        if (waifu) {
            ...
            await pool.query('UPDATE spawn_tracker SET active_spawn_waifu_id = $1, bid_message_count = 0 WHERE group_id = $2', [waifu.waifu_id, groupId]);
        }
    } else {
...
bot.onText(/\/grab(?:\s+(.+))?/, async (msg, match) => {
    ...
    // If user provided a guess, check if it matches
    if (match && match[1]) {
        const guess = match[1].trim().toLowerCase();
        const actualName = w.name.toLowerCase();
        
        if (!actualName.includes(guess) && !guess.includes(actualName)) {
            return sendReply(msg.chat.id, msg.message_id, `❌ Wrong guess! Try again.`);
        }
    }

    await pool.query('INSERT INTO harem (user_id, waifu_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [userId, waifuId]);
    await pool.query('UPDATE spawn_tracker SET active_spawn_waifu_id = NULL, message_count = 0 WHERE group_id = $1', [groupId]);
    await ensureUser(userId, msg.from.username, msg.from.first_name);
    await saveUserDataToFile(userId);
